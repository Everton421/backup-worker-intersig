...
export const sendBackupMessageRoute: FastifyPluginAsyncZod = async (server) => {
    server.post('/send-message-backup/:codigo', {
        preHandler: [
            checkRequest,
            checkUser('suport')
        ],
        ...
        async (request, reply) => {
            ...
            if (selectedClientBackup.length > 0) {
                if (selectedClientBackup[0].efetuar_backup && selectedClientBackup[0].efetuar_backup == 'S') {
                    ...
                    const pathBackups = 'C:/backups-api'
                    ...
                    if (conn !== null) {
                        ...
                        try {
                           ...
                           await sendBackupMessage({ 
                                   ...
                                   databases: databases ,
                                   pathZip:   caminhoBkp || pathBackups
                            });

                                reply.status(200).send({ msg: 'backup enviado para fila!' })
                                const config = {
